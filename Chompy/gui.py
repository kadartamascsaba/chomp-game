...
class ChompUI:
	def __init__(self, master, game):
		...
		self.start_button = Button(master, text="Start", command=self.initialize_UI)
		...
		for i in xrange(self.height):
			for j in xrange(self.width):
				...
				if (i == (self.height - 1)) and (j == 0):
					chocolate_bar = Button(frame, width=button_width, height=button_height, bg="Orangered4", command=self.get_button)
				else:
					chocolate_bar = Button(frame, width=button_width, height=button_height, bg="saddle brown", command=self.get_button)

				frame.grid(row=i, column=j, padx=5, pady=5)
				chocolate_bar.grid(sticky="NWSE")

	def get_button(self):
		x, y = self.buttons(self)
		game.
